...
class TestToolHandlers:
    ...
    def test_recall_expands_conflict_run_at_source_group_boundary(self, provider_with_config):
        ...
        p._client.arecall.return_value = SimpleNamespace(results=[
            ...
            SimpleNamespace(
                ...
                type="world", document_id="source-alpha", tags=["pilot", "conflict"],
            ),
            SimpleNamespace(
                text="Project Nimbus launch color is orange according to Source Beta.",
                type="world", document_id="source-beta", tags=["pilot", "conflict"],
            ),
            SimpleNamespace(
                text="The cafeteria served violet macarons.", type="world",
                document_id="irrelevant-source", tags=["pilot", "irrelevant"],
            ),
        ])
    ...
    def test_recall_query_gate_restores_earlier_conflict_source(self, provider_with_config):
        p = provider_with_config(
            recall_result_max_source_groups=1,
            recall_result_min_query_terms=2,
            recall_result_expand_conflicts=True,
        ...
        p._client.arecall.return_value = SimpleNamespace(results=[
            SimpleNamespace(
                text="Source Alpha recorded purple.",
                type="world", document_id="source-alpha",
                tags=["pilot", "conflict"],
            ),
            SimpleNamespace(
    ...
    def test_recall_world_restatement_cannot_discard_conflict_marker(self, provider_with_config):
        p = provider_with_config(
            recall_result_max_source_groups=1,
            recall_result_max_items=1,
            recall_result_expand_conflicts=True,
        ...
        p._client.arecall.return_value = SimpleNamespace(results=[
            SimpleNamespace(
                text="Project Nimbus launch color is purple according to Source Alpha.",
                type="observation", document_id="source-alpha",
                tags=["pilot", "conflict"],
            ),
            SimpleNamespace(
                text="Project Nimbus launch color is purple according to Source Alpha.",
    ...
    def test_recall_conflict_marker_wins_when_world_restatement_ranks_first(self, provider_with_config):
        p = provider_with_config(
            recall_result_max_source_groups=1,
            recall_result_max_items=1,
            recall_result_expand_conflicts=True,
    ...
    def test_formatter_preserves_legacy_tool_output_when_controls_disabled(self, provider_with_config):
        p = provider_with_config()
        results = [
            SimpleNamespace(text=" Memory 1 "),
        ...
        assert p._format_recall_results(results, numbered=True) == (
            "1.  Memory 1 \n2.  \n3. None"
        )

    def test_formatter_preserves_legacy_prefetch_output_when_controls_disabled(self, provider_with_config):
        p = provider_with_config()
        results = [
            SimpleNamespace(text=" Memory 1 "),
            SimpleNamespace(text=" "),
        ...
        assert p._format_recall_results(results, numbered=False) == "-  Memory 1 \n-  "
